    ...
    public void execute(CommandSender sender, Arguments arguments) {
        ...
        switch (action) {
            case SET -> {
                ...
                ChatMessage.info("Property " + TextUtil.getName(property) + " updated.").post(Main.PLUGIN_ID, sender);
            }
            case RESET -> {
                property.reset();
                ChatMessage.info("Property " + TextUtil.getName(property) + " has been reset.").post(Main.PLUGIN_ID, sender);
            }
            case INFO -> {
                ChatMessage.info(property.getDescription()).post(Main.PLUGIN_ID, sender);
                ChatMessage.info(TextUtil.getString(property)).post(Main.PLUGIN_ID, sender);
            }
        }
    }
